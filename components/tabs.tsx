...
function _Tabs({ items, selectedIndex: _selectedIndex, defaultIndex = 0, onChange, children, storageKey }: { items: (TabItem | TabObjectItem)[]; selectedIndex?: number; defaultIndex?: number; onChange?: (index: number) => void; children: ReactNode; storageKey?: string }): ReactElement {
      ...
      <div className="nextra-scrollbar nx-overflow-x-auto nx-overflow-y-hidden nx-overscroll-x-contain">
        <HeadlessTab.List className="nx-mt-4 nx-flex nx-w-max nx-min-w-full nx-border-b nx-border-gray-200 nx-pb-px dark:nx-border-neutral-800">
          {items.map((item, index) => {
            ...
            return (
              <HeadlessTab key={index} disabled={disabled} className={({ selected }) => cn('nx-mr-2 nx-rounded-t nx-p-2 nx-font-medium nx-leading-5 nx-transition-colors', '-nx-mb-0.5 nx-select-none nx-border-b-2', selected ? 'nx-border-primary-500 nx-text-primary-600' : 'nx-border-transparent nx-text-gray-600 hover:nx-border-gray-200 hover:nx-text-black dark:nx-text-gray-200 dark:hover:nx-border-neutral-800 dark:hover:nx-text-white', disabled && 'nx-pointer-events-none nx-text-gray-400 dark:nx-text-neutral-600')}>
...
export function Tab({ children, unmount = false, ...props }: Omit<ComponentProps<typeof HeadlessTab.Panel>, 'static'>): ReactElement {
  return (
    <HeadlessTab.Panel unmount={false} {...props} className="nx-rounded nx-pt-6">
      {children}
    </HeadlessTab.Panel>
  );
}
