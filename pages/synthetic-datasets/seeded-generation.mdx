...
description: 'With seeded generation, you can create a new synthetic dataset that is conditioned on a seed dataset you provide. This is also known as conditional generation.'
---
...
With **_seeded generation_**, you can create a new synthetic dataset that is conditioned on a seed dataset you provide. This is also known as **_conditional generation_**.
...
3. Under **Sample size**, select **seed**.
    <Image 
        src="/docs/docimages/synthetic-datasets/seeded-generation/01-sample-size-select-seed.webp" 
        alt="Seeded generation - Sample size - Select seed" 
        width={800} 
        height={150}
    />
4. Click **Upload seed file**.
    <Image 
        src="/docs/docimages/synthetic-datasets/seeded-generation/02-click-upload-seed-file.webp" 
        alt="Seeded generation - Sample size - Click Upload file" 
        width={800} 
        height={150}
    />
...
<Tabs.Tab>
If you use the Python SDK, use the `seed` parameter of `mostly.generate()` to specify a seed dataset.
...
The three examples below showcase what you can achieve with seeded generation.

<Callout>
The examples below are also available as a [Jupyter Notebook](https://colab.research.google.com/github/mostly-ai/mostly-tutorials/blob/dev/conditional-generation/conditional-generation.ipynb). You can also find it in our [Tutorials](/tutorials) section.
</Callout>

1. [Perform multi-column dataset rebalancing](#rebalance-datasets-with-seeded-generation). You create a seed dataset with two columns that have a new distribution. You then use the seed for conditional generation and see how the rest of the columns are impacted by the rebalancing.
...
* [MOSTLY AI Python SDK](https://pypi.org/project/mostlyai/)
...
1. Import the Python packages needed for this example.
    ...
    import numpy as np
    ```
1. Get an API key and instantiate the Python SDK. For details, see [here](/python-sdk#get-an-api-key).
    ```python filename="python" copy
    mostly = MostlyAI(api_key="INSERT_API_KEY")
    ```
2. Train a generator on the US Adult dataset or get an existing one.
    * Train a new generator.
        ```python filename="python" copy
        df = pd.read_csv('http://mostly.ai/docs/datasets/us-census-income.csv.gz')
...
3. Create a seed dataset.<br />
    ...
    ```
4. Generate a rebalanced dataset with the seed dataset as context.
    ```python filename="python" copy
    sd = mostly.generate(generator=g, seed=seed)
    ```
5.  Use [Matplotlib](https://matplotlib.org/) to compare the age distribution of records from the original vs rebalanced dataset.
    ```python filename="python" copy
...
<Image
    src="/docs/docimages/synthetic-datasets/seeded-generation/01-example-01-chart.webp"
    alt="Seeded generation - Example 01 - Chart - Rebalance dataset"
    width={450}
    height={500}
/>
...
6. Compare the partial synthetic data to the original data.
    ...
    ```
    <Image
        src="/docs/docimages/synthetic-datasets/seeded-generation/02-example-01-chart.webp"
...
1. Train a generator with a dataset containing names of people. For this example, we will use the **baseball** dataset.
    1. Create a new generator and add the `players.csv` table from the [baseball dataset](/datasets#baseball-dataset).
        <Image
            src="/docs/docimages/synthetic-datasets/seeded-generation/03-example-generator-baseball-text-encoding-type.webp"
            alt="Seeded generation - Example 03 - Generator with baseball/players table - names set to Language/Text"
            width={800}
            height={150}
        />
    ...
    4. For **Model**, make sure the **MOSTLY_AI/LSTMFromScratch** model is selected.
        <Image
            src="/docs/docimages/synthetic-datasets/seeded-generation/03-example-generator-configure-model-mostly-ai-lstm.webp"
            alt="Seeded generation - Example 03 - Configure model - MOSTLY_AI/LSTMFromScratch"
            width={800}
            height={150}
        />
...
2. Get the generator ID.
    1. With the generator open, click its kebab menu and select **Copy ID**.
        <Image
            src="/docs/docimages/synthetic-datasets/seeded-generation/03-example-generator-copy-id.webp"
            alt="Seeded generation - Example 03 - Generator - Copy ID"
            width={800}
            height={150}
        />
...
3. In a Jupyter Notebook, live-probe the generator for names from the country Mexico.
    <Callout>
    The **baseball** dataset contains players from the countries USA, Canada, Mexico, Cuba, and others. The goal is to demonstrate how the LSTM model is able to learn the names of players from specific countries.
    </Callout>
    1. Get the generator by ID.
        ```python filename="python" copy
        g = mostly.generators.get("INSERT_GENERATOR_ID")
        ```
    2. Create a seed with 3 rows of the country Mexico.
        ```python filename="python" copy
        seed = pd.DataFrame({
            'country': ['Mexico'] * 5
        })
        ```
    3. Live-prove the generator with the seed dataset.
        ```python filename="python" copy
...
With seeded generation, you can probe generators with a specific context, whether that is hypothetical ([example 1](#rebalance-datasets-with-seeded-generation)) or real ([example 2](#generate-partially-synthetic-geographical-data)), and gain corresponding insights for specific scenarios. You can also generate synthetic text based on the provided context ([example 3](#generate-synthetic-names)).
