...
[**Language models**](#language-models). Language models include the [MOSTLY_AI/LSTMFromScratch-3m](#mostly_ailstmfromscratch-3m) model and [HuggingFace LLMs](#huggingface-llms). With language models, you can generate privacy-preserving **synthetic text data**. Encoding types for language models are prefixed with **Language/** (when using the Platform UI) or `LANGUAGE_` (when using the Python SDK).
...
* train on short text data to later generate privacy-preserving names, emails, and so on.
...
* use the not pre-trained [MOSTLY AI/LSTM](#mostly_ailstmfromscratch-3m) model to synthesize unstructured natural language texts up to 1,000 characters
* fine-tune a [HuggingFace](https://huggingface.co/) LLM with the text data in the selected column.

<CustomCallout>
The list of HuggingFace LLMs is configurable and can be extended. If a model you want to use is not available, contact [MOSTLY AI Support](mailto:support@mostly.ai).
</CustomCallout>

## Models

MOSTLY AI supports the following models for generating synthetic tabular data and synthetic text.
...
MOSTLY AI supports fine-tuning HuggingFace LLMs with original text data.
