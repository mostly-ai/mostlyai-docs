...
1. Download the [Berka dataset](/datasets/berka.zip) and add all tables to a new generator.
    <Image
        ...
        height={300}
    />
2. Define foreign keys for all linked tables that refer the `account` table.
    1. Set a context foreign key from the `loan` table (`account_id` column) to the `account` table.
        <Image
            src="/docs/docimages/generators/set-table-relationships/multi-table-rel-02-set-context-for-loan-table.webp"
            alt="MOSTLY AI - Multi-table relationships - Set foreign key for loan table"
            width={900}
            height={300}
        />
    2. Set a context foreign key from the `order` table (`account_id` column) to the `account` table.
        <Image
            src="/docs/docimages/generators/set-table-relationships/multi-table-rel-03-set-context-for-order-table.webp"
            alt="MOSTLY AI - Multi-table relationships - Set foreign key for order table"
            width={900}
            height={300}
        />
    3. Set a context foreign key from the `transaction` table (`account_id` column) to the `account` table.
        <Image
            src="/docs/docimages/generators/set-table-relationships/multi-table-rel-04-set-context-for-transaction-table.webp"
            alt="MOSTLY AI - Multi-table relationships - Set foreign key for transaction table"
            width={900}
            height={300}
        />
    ...
    5. Set a context foreign key from the `card` table (`disp_id` column) to the `disp` table.
        <Image
            src="/docs/docimages/generators/set-table-relationships/multi-table-rel-06-set-context-for-card-table.webp"
            alt="MOSTLY AI - Multi-table relationships - Set foreign keys for card table"
            width={900}
            height={300}
        />
3. Set the primary keys for each table that doesn't already have one.
    1. Click the **Primary key** column for a table and select its primary key column.
    2. Repeat for each table.
        <Image
            src="/docs/docimages/generators/set-table-relationships/multi-table-rel-07-set-pk.webp"
            alt="MOSTLY AI - Multi-table relationships - Set foreign keys for card table"
            width={900}
            height={300}
        />
...
You can now take a look at the relationship diagram.
</Tabs.Tab>
<Tabs.Tab>
