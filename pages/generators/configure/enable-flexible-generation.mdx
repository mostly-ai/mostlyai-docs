...
| --- | --- |
| Enabled **Flexible generation** | Uses the **Any order** model of feeding original columns for training that is randomly permuted for every batch.<br /><br />The **Any order** model enables a generator to:<br /><br />  • [rebalance data](/synthetic-datasets/rebalance-columns)<br />  • [impute data](/synthetic-datasets/data-imputation)<br />  • [use fairness](/synthetic-datasets/fairness)<br />  • [generate synthetic data conditionally based on a seed dataset](/synthetic-datasets/seeded-generation)<br /><br />➡️ Flexible generation can lead to a reduced accuracy. Most impacted are models trained on particularly wide tables. |
