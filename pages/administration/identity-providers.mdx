...
4. Click **Add provider** and select **OpenID Connect v1.0**.
    <Image
        ...
        height={300}
    />
5. For **Alias**, enter `pingone`.
6. For **Display Name**, enter `PingOne`.
    <Callout>
    Note the rendered Redirect URI. This will be used in the PingOne configuration.
    </Callout>
7. For **Client ID**, enter the **Client ID** from the PingOne configuration.
8. For **Client Secret**, enter the **Client Secret** from the PingOne configuration.
9. For **Discovery endpoint**, enter the **OIDC Discovery Endpoint** from the PingOne configuration.
10. Click **Add**.
    <Image
...
1. Go back to Keycloak and open the **Ping One** identity provider.
