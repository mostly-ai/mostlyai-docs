...
<Image
  src="/docs/docimages/administration/backup-restore/velero-diagram.webp" 
  alt="Velero diagram"
  width={800}
  height={15000}
/>
...
Velero uses plugins to integrate with a variety of storage systems and Kubernetes platforms to support backup, restore, and snapshot operations. [Velero Plugins](https://velero.io/plugins/).
...
**NOTE:** You need to take care on create AWS S3 bucket, for backup destination storage, as well as crate AWS credentials and point it with `--secret-file` parameter. 

Format of `velero_credentials` file is:
...
With Velero in palce, you can now create a backup of MOSTLY AI.
...
2. Check the status of the backup and its progress:
    ...
    ```
    You should output similar to the one below.
    ```
...
velero backup describe mostly-ai-backup-poc
```
The backup information should be similar to the one listed below.
```
Name:         mostly-ai-backup-poc
Namespace:    velero
Labels:       velero.io/storage-location=default
...
The process below demonstrates how you can clean up the MOSTLY AI Kubernetes resources and restore MOSTLY AI from a backup.
...
1. List the Helm releases installed in your cluster.
    ...
    helm list
    ```
    If you have MOSTLY AI deployed in your cluster, you should see output similar to the one below.
    ```
...
2. Use `helm` to uninstall the MOSTLY AI release running in your cluster.
    ...
    ```
    If successful, the result from the command should be the following:
    ```
...
3. Check the status of the MOSTLY AI pods.
    ...
    kubectl get pods
    ```
    The MOSTLY AI pods should be back and stable.
    ```
    NAME                                  READY   STATUS    RESTARTS      AGE
