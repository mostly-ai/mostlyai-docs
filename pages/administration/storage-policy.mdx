...
The storage system of MOSTLY AI is a compatible S3 storage. Storage is only used to store the following:
...
The only place where original data is accessed is in the training job, which is a Kubernetes job. The original data is stored temporarily on the job’s volume, which is destroyed by Kubernetes when the job finishes.
