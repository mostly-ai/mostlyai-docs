...
To add resources, you can [share the resource from your personal account to the organization](#share-a-resource-with-an-organization). A resource's initial owner is always the creator's account. Resources cannot start with an organization as the owner, but they can always be shared back to your account.
...
> You cannot share resources with a single specific user or to an organization where you do not have sufficient permissions.
...
- You can share generators and synthetic datasets only when their status is **Ready**. For details, see [Status of generators](/generators/manage#status-of-generators) and [Status of synthetic datasets](/synthetic-datasets/manage#status-of-synthetic-datasets).

**Steps**

1. Open a resource (generator, synthetic dataset, connector, dataset).
2. Click **Share**.
...
| **Share the resource back to your account** | Follow [Share a resource to an organization](#share-a-resource-with-an-organization). Instead of an organization, select your account.                                                                                      |

As a result, the resource is no longer available to the organization members.
