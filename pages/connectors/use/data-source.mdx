...
Create a connector for a [database](../create#database-connectors) or [cloud storage bucket](../create#cloud-storage-connectors) and select **Source** under **Access type**. You can use all supported databases and cloud object storage providers as a data source for your original data.

<Image
    src="/docs/docimages/connectors/connector-source.webp"
    alt="Connector - Select Source under Connector type"
    width={350}
    height={30} 
/>
...
1. From the **Generators** page, open an untrained generator or create a new one by clicking **+ New generator**.
    <Image
        ...
        alt="Connector as a data source - 01 - Click New generator"
        width={800}
        height={30}
    />
3. Click **Add data**.
    <Image
        src="/docs/docimages/connectors/source/click-add-table.webp"
        alt="Connector as a data source - 02 - Click Add table"
        width={800}
        height={30}
    />
4. Click **Connect to source**.
    <Image
        src="/docs/docimages/connectors/source/train-generator-02-click-connect-to-source.webp"
        alt="Connector as a data source - 04 - Click Connect to source"
        width={800}
        height={30}
    />
5. Select a source connector from the list from which to add data.
    <Image
        src="/docs/docimages/connectors/source/select-source-connector.webp"
        alt="Connector as a data source - 05 - Select connector"
        width={800}
        height={30}
    />
6. Follow the steps depending on whether you use a [database connector](/connectors/create#database-connectors) or a [cloud storage connector](/connectors/create#cloud-storage-connectors).
    * For [**cloud storage connectors**](/connectors/create#cloud-storage-connectors), click in the **Select table** field to view and expand the bucket tree structure. Select a table or a folder containing a partitioned table (multiple `.parquet` files).
        <Image
            src="/docs/docimages/connectors/source/object-storage-tree-view-select-table.webp"
            alt="Connector as a data source - 06 - Object storage tree view - select table"
            width={800}
            height={30}
        />
    * For [**database connectors**](/connectors/create#database-connectors), select a schema (optional) and a table from the list.
        <Image
            src="/docs/docimages/connectors/source/database-select-schema-table.webp"
            alt="Connector as a data source - 06 - Database connector - select schema and table"
            width={800}
            height={30}
        />
7. (Optional) Select **Include child tables** to add any tables that reference the currently selected table with a foreign key.
...
<Tabs.Tab>
If you use the Python SDK, add the `name`, `location`, and `source_connector_id` to the table dictionary in the `tables` list.
