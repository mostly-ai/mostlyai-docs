...
1. In Google Cloud BigQuery, open the main sidebar menu and select **APIs & Services** > **Enabled APIs & services**.
    <Image
        src="/docs/docimages/connectors/bigquery/bigquery-apis-services.webp"
        alt="Google Cloud BigQuery - Select APIs and Services > Enabled APIs and services"
        width={350}
        height={500}
    />
2. From the sidebar, select **Credentials**.
    <Image
        src="/docs/docimages/connectors/bigquery/bigquery-sidebar-select-credentials.webp"
        alt="Google Cloud BigQuery - Select Credentials"
        width={175}
        height={500}
    />
3. Click your service account.
    <Image
        src="/docs/docimages/connectors/bigquery/bigquery-click-service-account.webp"
        alt="Google Cloud BigQuery - Click Service account"
        width={550}
        height={500}
    />
...
5. Click **ADD KEY** and select **Create new key**.
    <Image
        src="/docs/docimages/connectors/bigquery/bigquery-click-add-key-select-new-key.webp"
        alt="Google Cloud BigQuery - Click Add key and select New key"
        width={350}
        height={500}
    />
6. In the prompt, select **JSON** and click **Create**.
    <Image
        src="/docs/docimages/connectors/bigquery/bigquery-select-json-click-create.webp"
        alt="Google Cloud BigQuery - Select JSON and click Create"
        width={350}
        height={500}
    />
...
2. On the **Connect to cloud storage** tab, select **Google cloud storage**.
    <Image
        src="/docs/docimages/connectors/google-cloud/create-connector-02-select-google-cloud.webp"
        alt="Select Google Cloud storage connector"
        width={800}
        height={500}
    />
3. On the **Create Google cloud connector** page, configure the connector.
    ...
    3. For **Key file**, paste the contents of your Google Cloud Storage key file.
        <Image
            src="/docs/docimages/connectors/google-cloud/create-connector-03-configure-gcp-bucket.webp" alt="Configure Google Cloud Storage bucket connector"
            width={300}
            height={500}
        />
...
<Tabs.Tab>
If you use the Python SDK, create a Google Cloud storage connector with `mostly.connect()` and provide a connector configuration dictionary as shown below.
