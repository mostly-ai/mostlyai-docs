...
* Fine-tune LLMs with smaller training sample sizes to then leverage the knowledge of pre-trained LLMs for generating representative synthetic data.
* Apply [_conditional generation by seeding_](/synthetic-datasets/seeded-generation) categories, numeric values, or datetime values that weren't present in the original data to again leverage the LLM knowledge for representative generation.
