...
* {/* MSD-1837 */} Platform - Enhance resource sharing experience
* {/* MSD-1875 */} Platform - Make login page mobile responsive
* {/* MSD-1431 */} Assistant - Indicate if a previous streaming is ongoing after reload
* {/* MSD-1873 */} Assistant - Consider token usage for credit consumption
* {/* MSD-1887 */} Assistant - Rename Threads to Chats, incl URL routes
...
* {/* MSD-1744 */} Replace padding-utils
* {/* MSD-1821 */} Prevent click back to navigate to center-input page in a thread
* {/* MSD-1854 */} Assistant: Minor UI Improvements
* {/* MSD-1865 */} Make ws-service only proxy requests from app to UI
* {/* MSD-1876 */} Only show inputs if user can admin thread
* {/* MSD-1892 */} Removing Manage Visibility option in an unlisted chat for viewer user
* {/* MSD-xxxx */} Aligning the search bar and filter icon using items-center
* {/* MSD-xxxx */} Resolve critical and high severity CVE vulnerabilities
* {/* MSD-xxxx */} Skip FlexiGeneration check for conditional generation (just like SDK)
* {/* MSD-xxxx */} Refined system instructions
* {/* MSD-1844 */} Assistant - Add a spinner when reinitializing a session with file upload
* {/* MSD-1863 */} Assistant - Escape HTML tags from user messages
* {/* MSD-1864 */} Assistant - Add loading indicator while fetching data
...
* {/* MSD-XXXX */} Fix upload file error on Assistant threads
...
{/* MSD-1839 */}
You can now generate and view fully interactive HTML content directly inside Assistant threads. This lets you create rich visualizations, reports, or custom UI elements that are rendered and usable in real time — perfect for sharing insights or building dynamic tools within your conversations.
...
* {/* MSD-1836 */} Fixed EmptyDataError when uploading a file in an inactive session, improving the reliability of file uploads in Assistant threads.
...
* {/* MSD-1805 */} Added a plus button and Python mode to the Assistant input area on the homepage, enabling file uploads and resource access without navigating into a thread.
...
### Assistant artifacts
{/* MSD-1658 */}
The Assistant now supports **artifacts**, allowing it to display resources (like Generators or Synthetic datasets) and files (like CSV, PNG) directly in your threads. A more interactive and organized way to work with results.
...
* {/* MSD-1680 */} Added filters to the Assistant's Thread list page, making it easier to navigate threads.
...
* {/* MSD-1669 */} Assistant: Added collapsible Python blocks for better organization and readability of code in threads
...
* {/* MSD-1687 */} Fixed an issue where the number of parallel running threads wasn't immediately updated after deleting a thread
* {/* MSD-1691 */} Optimized response time of API endpoint for creating new threads
...
* {/* MSD-1630 */} Assistant: Centered the input box in empty threads for a cleaner and more intuitive user experience.
* {/* MSD-1628 */} Assistant: Redesigned the thread header to be thinner, freeing up more space for the actual conversation area and improving usability.
...
* {/* MSD-1643 */} Update totalVirtualCpuTime in Assistant threads based on websocket events.
* {/* MSD-1637 */} Prevent large horizontal scrolls in wide tables within Assistant threads.
...
* {/* MSD-XXX */} Fixed Virtual CPU time computation for re-started threads, ensuring accurate resource tracking and credits consumption.
...
{/* MSD-1581 */}
The Assistant now automatically restarts Python sessions that have become inactive. If your session times out, it will be reinitialized the next time you send a message. This allows you to continue the conversation in the same thread without manually starting a new session.
...
* {/* MSD-1550 */} Fixed vCPUTime updates for threads when kernel is not running
...
{/* MSD-1307 */}
MOSTLY AI is transitioning to a compute-based credit system for tracking usage. The new system is more accurate and fair, as it accounts for the actual compute resources used for training generators, generating synthetic datasets, and the usage in Assistant threads.
...
* {/* MSD-1418 */} Added usernames in front of Assistant thread names.
...
* {/* MSD-1377 */} Resolved the page refreshes after an Assistant thread title is updated
...
* {/* MSD-1297 */} Resolved an issue with the display of tables and images in Assistant threads
...
2. Generate synthetic data in your local environment.
    ...
    # 1. Import the Synthetic Data SDK and pandas
...
* {/* MSD-1035 */} Implemented a retry mechanism for file uploads in Assistant threads
...
* {/* MSD-1167 */} Window title now displayed correctly when adding a generator or synthetic dataset to an Assistant thread
...
* {/* MSD-1096 */} Moved the **Python mode** button in assistant threads to improve usability
...
* {/* MSD-1065 */} Use names of any length for all entities (generators, synthetic datasets, connectors, assistant threads, and computes). In the UI, the names are truncated and appear in full when you hover over them.

### Resolved issues

* {/* MSD-1108 */} Resolved an error while trying to fetch an image file from an Assistant thread
...
You can now run your assistant threads in a Jupyter Notebook environment. All assistant threads are now exportable `.ipynb` files.

For details, see [_Export as Jupyter Notebooks_](/assistant/manage-threads#export-as-a-jupyter-notebook).
...
- {/* MSD-867 */} The `createShare` API endpoint is now available only to users who have Editor permission for a generator, synthetic dataset, or assistant thread.
...
<Image
    src="/docs/docimages/release-notes/v228-job-failure-heartbeat-error.webp"
    alt="MOSTLY AI - Job failure error"
    width={800}
    height={500}
/>
...
{/* MSD-??? */}
Each Assistant thread now includes information about the number of tokens used in the thread. Tokens are counted both for the number of tokens sent and received. Tokens used in Assistant threads count towards your daily credits usage.

Tokens are counted in threads starting from version 227. Threads created before this version will always have their tokens used count shown as 0.
...
<Image
    src="/docs/docimages/release-notes/v228-download-logs-failed-job.webp"
    alt="MOSTLY AI - Download logs for failed jobs"
    width={800}
    height={500}
/>
...
* {/* MSD-760 */} Introduced heartbeat messages for Assistant threads to avoid unexpected timeouts and incomplete Python code execution
...
### Easiest way to start Assistant threads
{/* MSD-764 */}

The **New thread** button is now available directly from the sidebar. You can now start new Assistant threads from anywhere in the app.
...
<Image
    src="/docs/docimages/release-notes/v223-search-drop-downs-schemas-tables.webp"
    alt="MOSTLY AI - Search database schemas and tables"
    width={800}
    height={500}
/>
...
### Interactive data tables in Assistant threads

Assistant threads now support the display of tabular data in interactive tables. The interactions are available directly whenever the Assistant displays a data table. You can sort, search, paginate, and download the data in CSV format.

### Run your Python code in Assistant threads

You can now run your own Python code in Assistant threads. For all those cases where you need to run your custom code and not the one generated by the Assistant, you can now paste your Python snippet directly and the Assistant will run it for you.
...
Admins can now share Generators, Synthetic datasets, Connectors and Assistant threads with all users in their organization. 
...
* {/* MSD-701 */} Fix for an issue that prevented uploading a file in Assistant threads for some users.
...
### File uploads in Assistant threads
...
You can now upload files in Assistant threads. You can do so with single or multiple files. Instant examples are typical data file types, such as CSV, Parquet, or Excel. But you can use any file type that an LLM can process.

For details, see [Upload files in Assistant threads](/assistant#upload-files).

### Add MOSTLY AI assets to Assistant threads

While you can use the **Explore** button from any generator or synthetic dataset to open the Assistant, you can now also add these assets directly to an Assistant thread. This way, you can easily refer to them in a conversation.
...
### Python session status in Assistant threads

Each Assistant thread also runs a Python session in the background to run any Python code generated by the Assistant. That session expires after 15 minutes of inactivity. You can now track your Python session status in each thread. The session now is started as soon the thread is created.
...
<Image
    src="/docs/docimages/common/redesigned-navigation.gif"
    alt="MOSTLY AI - Redesigned navigation"
    width={800}
    height={500}
/>
...
### Auto-generated titles for Assistant threads
{/* MSD-604 */}
After the second message in a thread, the Assistant now automatically generates a title for the thread based on the conversation content. You can now identify and find Assistant threads more easily based on the their conversation context and name.
...
* **Italics** - wrap in single asterisk of underscore, *italic* or _italic_
...
| **Speed**    | <ul><li>**Maximum training epochs** is set to **10**</li><li>**Training samples** is set to **100000**</li></ul> |
| **Turbo**    | <ul><li>**Maximum training epochs** is set to **1**</li><li>**Training samples** is set to **10000**</li></ul> |
