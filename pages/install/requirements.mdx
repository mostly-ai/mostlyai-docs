...
MOSTLY AI is a multi-service application that can run on single-node or multi-node Kubernetes clusters. For a successful deployment and operation, the Kubernetes cluster on which you deploy MOSTLY AI must meet the compute and storage requirements. The minimum [Kubernetes version](https://kubernetes.io/releases/) supported is **>1.29**.
