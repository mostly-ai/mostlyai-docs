...
<Callout>
NGINX installation may be required only for [GKE](../../deploy/gke) and [Azure](../../deploy/azure) deployments.
</Callout>
...
## Configure NGINX in the MOSTLY AI Helm chart

Define NGINX as the ingress controller and then enable the required `ingress` annotations.

**Steps**

1. Assign the `ingressClassName` to be `nginx` under `_customerInstallation.deploymentSettings`.
