...
## How are connector secrets (passwords, tokens, secret keys, etc.) stored in MOSTLY AI?

The secrets for each saved connector in MOSTLY AI are stored in the PostgreSQL database that is part of the [MOSTLY AI application services](/install/architecture). The secrets are encrypted not only at rest, but also in transit.