...
The secrets (passwords, API keys, secret keys, and so on) for each saved connector in MOSTLY AI are stored in encrypted form in the PostgreSQL database that is part of the [MOSTLY AI application services](/install/architecture#application-nodes). The secrets are not only encrypted at rest, but also in transit.
